...
router.get('/releases', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const selectCols: string[] = [
      ...
      'sd.user_id',
      'u.full_name',
      "CASE WHEN ad.admin ~ '^[0-9]+$' THEN (SELECT full_name FROM user_tbl WHERE user_id = CAST(ad.admin AS INTEGER) LIMIT 1) ELSE ad.admin END AS admin",
      'ad.date AS created_at',
      'r.status',
