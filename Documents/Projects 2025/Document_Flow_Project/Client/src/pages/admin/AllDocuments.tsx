  ...
  const fetchAllDocuments = useCallback(async () => {
    ...
    try {
      ...
      const mappedReceived: Document[] = (received || []).map((r: ReceivedRaw, idx: number) => ({
        ...
        User_Id: r.user_id ?? 0,
        // For received requests, the UI derives the visible status from `mark` (Done/Not Done)
        // and we display the admin name in the description column.
        Status: ('Released' as Document['Status']),
  ...
  const renderReceivedActions = (doc: Document) => {
    ...
    return (
      <Button
        ...
        aria-label={`Respond to ${doc.Type}`}
      >
